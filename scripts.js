...
// Google Maps API Key
const apiKey = 'AIzaSyCmKBQVwmgMV8kVij7qqOiBQNsJIbA5qwc';

// Paths to JSON data
const markersFeed = 'https://www.4000massaveapts.net/dev/wp-json/wp/v2/area_landmarks?per_page=10';
const catsFeed = 'https://www.4000massaveapts.net/dev/wp-json/wp/v2/landmark_types';

// Specify whether you want to add a static community marker to the map (true/false).
const addCommMarker = false;
const locationOptionsFeed = 'https://www.4000massaveapts.net/dev/wp-json/acf/v2/options';
...
document.addEventListener('DOMContentLoaded', function() {
	if(document.getElementById(myMap)) {
		...
		mapJsFile.src = 'https://maps.googleapis.com/maps/api/js?callback=fetchData&language=' + lang;
...
function buildMap(data, location) {
	...
	for(let i = 0; i < data.length; i++) {
		...
		let marker = new google.maps.Marker({
			...
			map: map,
			//icon: image,
			html: `<strong class="heading">${data[i][1]}</strong>${data[i][4]}${data[i][5]}${data[i][6]}${data[i][7]}${data[i][8]}`,
		});
		markers.push(marker);
...
function buildCats(data) {
	...
	function catClick(href, ul) {
		href.addEventListener('click', function(event) {
			...
			const listItems = Array.from(document.querySelectorAll(`#${landmarkListWrapper} li`));
